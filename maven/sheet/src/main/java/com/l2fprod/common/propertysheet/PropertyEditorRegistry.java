...
public class PropertyEditorRegistry implements PropertyEditorFactory {
  ...
  private static final Logger LOGGER = Logger.getLogger(PropertyEditorRegistry.class.getName());
   ...
   * </ul>
   * 
   * @param property the Property to lookup a PropertyEditor for
  ...
  private PropertyEditor loadPropertyEditor(Class<?> clz) {
    try {
      ...
      if (obj instanceof PropertyEditor) {
        ...
        LOGGER.log(Level.WARNING, "Class {0} is not a PropertyEditor", clz.getName());
      }
    } catch (Exception e) {
      LOGGER.log(Level.WARNING, "Failed to instantiate property editor " + clz, e);
    }
    return null;
  }
   ...
   * </ul>
   * 
   * @param type the property type to lookup an editor for
  ...
  public synchronized PropertyEditor getEditor(Class<?> type) {
    ...
    if (value instanceof PropertyEditor) {
      ...
      try {
        editor = (PropertyEditor)((Class<?>)value).getDeclaredConstructor().newInstance();
      } catch (Exception e) {
        LOGGER.log(Level.WARNING, "Failed to instantiate registered editor class", e);
      }
    }
    return editor;
  }
  ...
  public void registerDefaults() {
    ...
    //
    // Date editors based on what we have in the classpath: prefer JCalendar, fall back to NachoCalendar
    try {
      Class.forName("com.toedter.calendar.JDateChooser");
      Class<?> jcal = Class.forName("com.l2fprod.common.beans.editor.JCalendarDatePropertyEditor");
      registerEditor(Date.class, jcal);
    } catch (ClassNotFoundException e) {
      // try NachoCalendar
      try {
        Class.forName("net.sf.nachocalendar.components.DateField");
        Class<?> nacho = Class.forName("com.l2fprod.common.beans.editor.NachoCalendarDatePropertyEditor");
        registerEditor(Date.class, nacho);
      } catch (ClassNotFoundException ex) {
        // No date editor found on classpath
      }
    }
  }
